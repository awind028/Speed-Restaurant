...
public class OrderServiceImpl implements OrderService {
    ...
    private List<OrderVO> getOrderVOList(Page<Orders> page) {
        ...
        if (page != null && !page.isEmpty()) {
            for (Orders orders : page) {
                ...
                List<String> orderDishes = orderDetailList.stream().map(x -> x.getName() + "*" + x.getNumber() + ";").collect(Collectors.toList());
                orderVO.setOrderDishes(String.join("", orderDishes));
    ...
    public void repetition(Long id) {
        Long userId = BaseContext.getCurrentId();
        shoppingCartMapper.deleteByUserId(userId);
        List<OrderDetail> orderDetailLisl = orderDetailMapper.getByOrdersId(id);
        List<ShoppingCart> shoppingCartList = orderDetailLisl.stream().map(x -> {
    ...
    public PageResult conditionSearch(OrdersPageQueryDTO ordersPageQueryDTO) {
        ...
        List<OrderVO> list = getOrderVOList(orders);
    ...
    public void rejection(OrdersRejectionDTO ordersRejectionDTO) throws Exception {
        ...
        if (ordersDB != null && ordersDB.getStatus().equals(Orders.REFUND)) {
            Orders orders = Orders.builder()
                    ...
                    .payStatus(Orders.REFUND)   //退款
                    .rejectionReason(ordersRejectionDTO.getRejectionReason())   //拒单原因
                    .status(Orders.CANCELLED)   //修改订单状态
                    .cancelTime(LocalDateTime.now())    //修改取消时间
                    .cancelReason(ordersRejectionDTO.getRejectionReason())  //取消原因
                    .build();
            orderMapper.update(orders);
        } else {
            throw new OrderBusinessException(MessageConstant.ORDER_STATUS_ERROR);
